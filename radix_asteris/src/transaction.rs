...
pub async fn handle_cash(
    ...
    match database::get_prices(items.keys().cloned().collect()).await {
        Ok(prices) => {
            ...
            if difference > 0 {
                match database::create_partial_transaction(tx_id, items.clone(), difference as u32)
                    .await
                {
                ...
                if let Err(e) =
                    database::log_transaction(tx_id, items, difference.abs() as u32).await
                {
                    eprintln!(
...
pub async fn handle_credit(
    ...
    match database::get_account(account_id).await {
        Ok(account) => match database::get_prices(items.keys().cloned().collect()).await {
            ...
            Err(e) => Err(format!("Error querying items in database: {}", e)),
        },
