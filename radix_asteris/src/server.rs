...
use crate::{database, model::{Account, Item, TransactionMethod, TransactionRequest, TransactionStatus}, transaction::{handle_cash, handle_credit}};
...
pub async fn transaction(headers: HeaderMap, Json(payload): Json<TransactionRequest>) -> Result<Json<TransactionStatus>, String> {
...
pub async fn get_account(headers: HeaderMap, Path(account_id): Path<u32>) -> Result<Json<Account>, String> {
    if check_auth(headers) == false {
        return Err("Unauthorized".to_string());
    }
...
pub async fn insert_account(headers: HeaderMap, Json(payload): Json<Account>) -> Result<Json<String>, String> {
    if check_auth(headers) == false {
        return Err("Unauthorized".to_string());
    }
...
pub async fn sync(headers: HeaderMap) -> Result<Json<Vec<Item>>, String> {
    ...
    database::get_all_items().await.map(|items| Json(items)).map_err(|e| e.to_string())
}

pub fn check_auth(headers: HeaderMap) -> bool {
    match headers.get("x-auth-token") {
        Some(val) => if val.to_str().unwrap_or_default() != *AUTH_KEY {
            false
        } else {
            true
        }, 
